(function (G) {
  ...
  var state = {
    ...
    warnings: [],
    history: [],
    pointer: { x: 0, y: 0, down: false, moveSquad: false, fireHold: false, altHold: false, touch: false },
    keys: {},
    skillSlot: 0,
    moveDir: { x: 0, y: -1 },
    dashT: 0,
  ...
  function startPlay() {
    ...
    state.pointer.x = state.squad.x;
    state.pointer.y = state.squad.y;
    showScreen("play");
    G.audio.sync(state, 0);
    syncHud();
  }
  ...
  function syncHud(dt) {
    ...
    for (var s = 0; s < act.length; s++) {
      ...
      slot.type = "button";
      var ready = act[s].activeCd <= 0;
      var meta = G.activeMeta(act[s].def.active.id);
      var frac = ready ? 100 : Math.max(0, 1 - act[s].activeCd / act[s].def.active.cd) * 100;
      var n = act[s].stackN || 1;
      var sel = s === (state.skillSlot | 0);
      slot.className = "active-slot" + (ready ? " ready" : " cd") + (sel ? " selected" : "");
  ...
  function drawFavorAuras() {
    ...
    for (var u = 0; u < state.units.length; u++) {
      ...
      if (unit.hp <= 0 || seen[unit.id]) continue;
      var match = false;
  ...
  function drawAim() {
    ...
    ctx.strokeStyle = state.pointer.fireHold ? "rgba(255,180,90,0.35)" : "rgba(126,200,255,0.22)";
    ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  function draw() {
    ...
    if (worldA > 0.02) {
      ...
      for (var au = 0; au < state.units.length; au++) {
        if (!state.units[au].commander) G.drawPlayerUnit(ctx, state.units[au], state.time);
      }
    ...
    if (state.run && state.run.tempShield > 0 && worldA > 0.02) {
      ...
      for (var sh = 0; sh < state.units.length; sh++) {
        if (state.units[sh].hp <= 0) continue;
        ctx.beginPath();
